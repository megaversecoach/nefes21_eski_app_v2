...
export default {
    ...
    selectedDimensionId: {
      handler: function () {
        if (this.selectedDimensionId !== '') {
          ...
          this.$store.dispatch('todays/filterTodaySeriesPool', {
  ...
  computed: {
    ...
    layoutObjects: function () {
      return this.$store.getters['todays/filteredSeriesPool']
    }
  },
  methods: {
    ...mapActions({
      getTodaySeriesPool: 'todays/getTodaySeriesPool',
      getDimensions: 'layouts/getDimensions',
      getContents: 'content/getContents'
    }),
    async saveData() {
      const result = await this.$store.dispatch('todays/saveSeriesPool', {
    ...
    deleteContentTapped(data) {
      this.$store.dispatch('todays/deleteFromTodaySeriesPool', {
    ...
    saveAndCloseModal() {
      ...
      if (this.v$.$error) return
        ...
        this.$store.dispatch('todays/addContentToTodaySeriesPool', {
  ...
  beforeRouteLeave() {
    this.$store.dispatch('todays/clearFilteredSeriesPool')
  },
  created() {
    this.$store.dispatch('todays/clearFilteredSeriesPool')
    this.selectedDimensionId = ''
    this.selectedDimensionLabel = ''
    this.getTodaySeriesPool()
    this.getDimensions()
